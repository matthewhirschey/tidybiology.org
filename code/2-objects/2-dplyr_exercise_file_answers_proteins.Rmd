...
class(proteins)
class(proteins$length) #one possibility
```
...
proteins %>% 
  select(c("uniprot_id", "sequence", "length", "mass"))
```
...
proteins %>% 
  filter(protein_name == "Collagen alpha-1") %>% 
  filter(length > 1000) 
```
...
proteins %>% 
  arrange(gene_name_alt) #only one example, could have chosen any except length or mass
```
...
write_csv(size_by_letter, "size_by_letter.csv")
```
