...
#It's the same, but follows the recommended style guidelines.
```
...
#new
ggplot(mitocarta) + 
	geom_point(aes(training_dataset,log(protein_length)))
```
...
ggplot(mitocarta, aes(x=hg19_chromosome, y=log(protein_length)))+
  ...
  geom_boxplot(aes(color=hg19_chromosome))+
	geom_label(data=large_prot, aes(label= symbol))+
	labs(x= "Chromosome")+
  guides(color= "none") #several ways to do this, here is one way
```
...
ggplot(mitocarta, aes(x=hg19_chromosome, y=log(protein_length)))+
  ...
  geom_boxplot(aes(color=hg19_chromosome))+
	geom_label(data=large_prot, aes(label= symbol))+
	labs(x= "Chromosome", 
	     y = "Protein Length (log)", 
	     title = "Distribution of mitochondrial protein length across chromosomes")+
  guides(color= "none")
```
...
ggplot(mitocarta) + 
  ...
  coord_flip()+
  labs(x = "Chromosome", y = "Protein Length (log)", title = "Lengths of proteins encoded by each Chromosome")+
  theme_linedraw() #many options here; just choose one. You can find options in the documentation
```
