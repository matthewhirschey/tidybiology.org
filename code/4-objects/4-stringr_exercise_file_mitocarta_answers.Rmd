...
```{r}
proteins %>% 
  dplyr::mutate(e_box = str_detect(sequence, "CA..TG")) %>% 
  dplyr::relocate(e_box) # moves the named column to the start of the data frame; not required, but useful
```

What are the mean protein lengths for proteins that contain an E-box versus proteins that do not? Hint: use the code you wrote in the last code chunk and build upon it.
```{r}
proteins %>% 
